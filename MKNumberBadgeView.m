...
- (void)initState;
{
    ...
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapOnBadgeView:)];
    [self addGestureRecognizer:tap];
    [tap release];
}

- (void)tapOnBadgeView:(id)sender
{
    ...
    if(self.superview && [self.superview isKindOfClass:[UIButton class]])
    {
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.shadow)
    {
        ...
        blurSize.width = 0;
        blurSize.height = 1;
        UIColor* blurColor = [[UIColor blackColor] colorWithAlphaComponent:0.3];
