...
NSData* hexToBinWithString(NSString* txt)
{
	...
	return [mData autorelease];//memory leak
}
...
+ (NSString*)encryptPwd:(NSString*)pwd
{
	...
	if([pwd length]>0)
	{
		...
		NSString* keyStr=@"A16AEC8FB8612156308AAFADEAE7794EE081E7676AF9B0079C57602AB8E1105B";
...
+ (NSString*)decryptPwd:(NSString*)encryptPassword
{
	if([encryptPassword length]>0)
	{
		...
		return [[[NSString alloc] initWithData:srcData encoding:NSUTF8StringEncoding] autorelease];
	}
	return nil;
}
...
+ (NSString *)getDeviceUniqueMD5
{
    ...
    if (!macaddressMd5) {
        ...
        macaddressMd5 = [[self MD5DigestFromString:macaddress] retain];
    }
...
{
    return [MyTool MD5Digest:[aString dataUsingEncoding:NSUTF8StringEncoding]];
}
...
+ (NSString*)MD5RandomStringFromTime:(NSDate*)cTime extraNum:(BOOL)bNeed
{
    ...
    NSString* rtval = [MyTool MD5DigestFromString:[NSString stringWithFormat:@"%f",timeNum]];
    return rtval;
}
...
+ (NSString*)writeToDocument:(id)data folder:(NSString*)folderName fileName:(NSString*)filename
{
    ...
    NSLog(@"test filepathName=%@",filepathName);
